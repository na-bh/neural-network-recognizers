"""Full-sweep per-cell decomposition analysis for compute-sqrt, matching the
...
overall accuracy plus three balanced-discrimination criteria:
  ...
  (c) exceeds_ceiling_balanced: overall_accuracy > THEORETICAL_CEILING
      (from fix_baseline_ceiling_compute_sqrt.json) AND balanced_any
...
not a uniform one -- CSF1's audit found low-tertile flips average bit_idx
~13 (magnitude change ~2^13) while high-tertile flips average bit_idx ~69
(magnitude change ~2^69, at the longest test sequences up to bit_idx 344).
...
negatives is only catching SMALL-MAGNITUDE errors (a claimed answer off by
a few units), not doing genuine bit-exact sqrt verification. Only balanced
...
documented for binary-addition/binary-multiplication/compute-sqrt's Phase 3
causal-patching pilot), not comprehensive correctness checking. This
magnitude-scaling relationship (bit_idx, magnitude=2**bit_idx) is reported
per tertile below (magnitude_scaling_by_tertile, a property of the TEST
SET's hard-negative population, not of any individual trained model).

PYTHONPATH=src:analysis python analysis/fix_full_sweep_analysis_compute_sqrt.py
"""
...
FIXED_LANG_DIR = REPO_ROOT / "languages" / "compute-sqrt-fixed"
...
def load_theoretical_ceiling():
    d = json.loads((RESULTS / "fix_baseline_ceiling_compute_sqrt.json").read_text())
...
def main():
    ...
    out_path = RESULTS / "fix_full_sweep_analysis_compute_sqrt.json"
    ...
    def save():
        crit = {
            ...
            "d_flat_across_tertiles_within_0.2": (
                ...
                "just balanced_any) to credit a cell with genuine bit-exact verification, since "
        ...
        out = {
            ...
            "magnitude_scaling_note": (
                ...
                "is only catching small-magnitude errors, not doing bit-exact sqrt verification."
            ),
    ...
    # ------------------------------------------------------------------
    for objective in OBJECTIVES:
        run_file = RESULTS / f"fix_trained_models_compute_sqrt_{objective.replace('+', '_')}.json"
    ...
    # ------------------------------------------------------------------
    std_file = RESULTS / "fix_standard_baseline_on_corrected_test_all_objectives_compute_sqrt.json"
    if std_file.exists():
