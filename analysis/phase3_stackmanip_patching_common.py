"""Shared causal-patching runner functions for stack-manipulation's bounded
...
prior task in this pilot (dyck-2-3, bucket-sort, missing-duplicate-string).

Uses separate clean/corrupt readout positions throughout (not assumed
equal) since Condition 1's malformed_operations/marker_count pairs are
same-length but length_mismatch pairs (Condition 2) are NOT.

Checkpoints: rec+ns/validation-short/<seed>, matching every other task's
Phase 3 patching in this pilot.

PYTHONPATH=src:analysis python analysis/phase3_stackmanip_patching_common.py
"""
...
CELLS = [
    ("rnn", 4, "primary"), ("rnn", 1, "secondary"),
    ("lstm", 1, "primary"), ("lstm", 7, "secondary"),
    ("transformer", 5, "primary"), ("transformer", 3, "secondary"),
    ("mamba", 3, "primary"), ("mamba", 1, "secondary"),
]
...
def load_cell_selection_context():
    d = json.loads((RESULTS / "phase3_stackmanip_cell_selection.json").read_text())
