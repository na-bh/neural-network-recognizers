"""Full-sweep per-cell decomposition analysis for missing-duplicate-string,
matching the Dyck-2-3/modular-arithmetic-simple/marked-copy template: for
...
on the corrected test set to recover PER-EXAMPLE correctness, then reports
...
criteria:
  ...
  (c) exceeds_ceiling_balanced: overall_accuracy > THEORETICAL_CEILING
      (from fix_baseline_ceiling_missing_duplicate_string.json) AND balanced_any

Also decomposes hard-negative accuracy by swap-POSITION tertile (low/mid/
high, read DIRECTLY from hard-negative-swap-meta.jsonl's swap_position_
tertile field -- MDF1's construction swept this explicitly via fixed-
target-tertile-then-retry, unlike marked-copy's post-hoc-bucketed anchor
position) -- checking whether the fix's effect (where present) is uniform
across early/mid/late swap positions or concentrated in some.

PYTHONPATH=src:analysis python analysis/fix_full_sweep_analysis_missing_duplicate_string.py
"""
...
def load_theoretical_ceiling():
    d = json.loads((RESULTS / "fix_baseline_ceiling_missing_duplicate_string.json").read_text())
...
def main():
    ...
    out_path = RESULTS / "fix_full_sweep_analysis_missing_duplicate_string.json"
    ...
    def save():
        ...
        out = {
            ...
            "experiment": (
                ...
                "discrimination, and is any such effect uniform across early/mid/late swap "
    ...
    # ------------------------------------------------------------------
    for objective in OBJECTIVES:
        run_file = RESULTS / f"fix_trained_models_missing_duplicate_string_{objective.replace('+', '_')}.json"
    ...
    # ------------------------------------------------------------------
    std_file = RESULTS / "fix_standard_baseline_on_corrected_test_all_objectives_missing_duplicate_string.json"
    if std_file.exists():
