"""Binary-multiplication Phase 3 (bounded) pilot: cell selection. Pulls
trained-architecture accuracies from flare_a2_binary_multiplication.json
...
checks for single-solver bottlenecks (matching phase3_dyck/missdup/
stackmanip/computesqrt_cell_selection.py's discipline exactly).

PYTHONPATH=src:analysis python analysis/phase3_binmult_cell_selection.py
"""
...
def select_cells():
    flare_a2 = json.loads((RESULTS / "flare_a2_binary_multiplication.json").read_text())
...
def main():
    ...
    print("\n=== hard-vs-scrambled accuracy (reported for context) ===", flush=True)
    print(json.dumps(hard_vs_scrambled, indent=2, default=str))

    out = {
        "task": TASK,
        "description": (
            "Cell selection for binary-multiplication's bounded Phase 3 patching pilot: primary "
            "(best) + secondary (next-best) seed per architecture, pulled from the existing "
            "flare_a2_binary_multiplication.json (no new training). Checks for per-seed single-"
            "solver bottlenecks AND architecture-wide near-chance collapse."
        ),
        ...
        "hard_vs_scrambled_negative_accuracy": hard_vs_scrambled,
        "final_cells": [
    ...
    out_path = RESULTS / "phase3_binmult_cell_selection.json"
