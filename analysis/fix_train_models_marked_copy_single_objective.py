"""Corrected-dataset experiment for marked-copy, MCF3: trains RNN/LSTM/
Transformer/Mamba x 10 seeds (40 runs) under ONE specified loss-term
objective on the corrected training set (languages/marked-copy-fixed),
...
objectives_marked_copy.py), not embedded here.

PYTHONPATH=src:analysis python analysis/fix_train_models_marked_copy_single_objective.py --objective rec
PYTHONPATH=src:analysis python analysis/fix_train_models_marked_copy_single_objective.py --objective rec+lm
PYTHONPATH=src:analysis python analysis/fix_train_models_marked_copy_single_objective.py --objective rec+ns
PYTHONPATH=src:analysis python analysis/fix_train_models_marked_copy_single_objective.py --objective rec+lm+ns
"""
...
FIXED_LANG_DIR = REPO_ROOT / "languages" / "marked-copy-fixed"
STANDARD_LANG_DIR = REPO_ROOT / "languages" / "marked-copy"
MODELS_ROOT = REPO_ROOT / "models" / "marked-copy-fixed"
TASK = "marked-copy"
...
def main():
    ...
    out_path = RESULTS / f"fix_trained_models_marked_copy_{slug}.json"
    ...
    results = {
        ...
        "experiment": "corrected-dataset-fix (MCF3: single-objective training on corrected data)",
        "description": (
            f"Trains RNN/LSTM/Transformer/Mamba on the corrected marked-copy training set "
            f"(languages/marked-copy-fixed) under the '{objective}' objective, 10 trials each (40 "
    ...
    for arch in ARCHITECTURES:
        for trial_no in range(1, N_TRIALS + 1):
            if (arch, trial_no) in done_keys:
                continue
            print(f"\n=== training marked-copy (corrected) {arch} {objective} trial {trial_no} ===", flush=True)
