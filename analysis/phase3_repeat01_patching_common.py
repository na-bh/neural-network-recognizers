"""Shared causal-patching runner functions for repeat-01's bounded Phase 3
...
Uses separate clean/corrupt readout positions throughout (Condition 1's
length_parity/null_control pairs are NOT same-length).

Checkpoints: rec+ns/validation-short/<seed>, matching every other task's
Phase 3 patching in this pilot.

PYTHONPATH=src:analysis python analysis/phase3_repeat01_patching_common.py
"""
...
CELLS = [
    ("rnn", 3, "primary"), ("rnn", 4, "secondary"),
    ("lstm", 1, "primary"), ("lstm", 2, "secondary"),
    ("transformer", 4, "primary"), ("transformer", 3, "secondary"),
    ("mamba", 1, "primary"), ("mamba", 3, "secondary"),
]
...
def load_cell_selection_context():
    d = json.loads((RESULTS / "phase3_repeat01_cell_selection.json").read_text())
