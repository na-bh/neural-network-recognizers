"""Corrected-dataset experiment for stack-manipulation, SMF3: trains RNN/LSTM/
Transformer/Mamba x 10 seeds (40 runs) under ONE specified loss-term
objective on the corrected training set (languages/stack-manipulation-fixed),
...
objectives_stack_manipulation.py), not embedded here.

PYTHONPATH=src:analysis python analysis/fix_train_models_stack_manipulation_single_objective.py --objective rec
PYTHONPATH=src:analysis python analysis/fix_train_models_stack_manipulation_single_objective.py --objective rec+lm
PYTHONPATH=src:analysis python analysis/fix_train_models_stack_manipulation_single_objective.py --objective rec+ns
PYTHONPATH=src:analysis python analysis/fix_train_models_stack_manipulation_single_objective.py --objective rec+lm+ns
"""
...
STANDARD_LANG_DIR = REPO_ROOT / "languages" / "stack-manipulation"
MODELS_ROOT = REPO_ROOT / "models" / "stack-manipulation-fixed"
...
def main():
    ...
    out_path = RESULTS / f"fix_trained_models_stack_manipulation_{slug}.json"
    ...
    results = {
        ...
        "experiment": "corrected-dataset-fix (SMF3: single-objective training on corrected data)",
        "description": (
            f"Trains RNN/LSTM/Transformer/Mamba on the corrected stack-manipulation training set "
            f"(languages/stack-manipulation-fixed) under the '{objective}' objective, 10 trials each (40 "
    ...
    for arch in ARCHITECTURES:
        for trial_no in range(1, N_TRIALS + 1):
            if (arch, trial_no) in done_keys:
                continue
            print(f"\n=== training stack-manipulation (corrected) {arch} {objective} trial {trial_no} ===", flush=True)
