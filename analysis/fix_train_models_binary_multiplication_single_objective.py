"""Corrected-dataset experiment for binary-multiplication, BMF3: trains RNN/LSTM/
Transformer/Mamba x 10 seeds (40 runs) under ONE specified loss-term
objective on the corrected training set (languages/binary-multiplication-fixed),
...
objectives_binary_multiplication.py), not embedded here.

PYTHONPATH=src:analysis python analysis/fix_train_models_binary_multiplication_single_objective.py --objective rec
PYTHONPATH=src:analysis python analysis/fix_train_models_binary_multiplication_single_objective.py --objective rec+lm
PYTHONPATH=src:analysis python analysis/fix_train_models_binary_multiplication_single_objective.py --objective rec+ns
PYTHONPATH=src:analysis python analysis/fix_train_models_binary_multiplication_single_objective.py --objective rec+lm+ns
"""
...
STANDARD_LANG_DIR = REPO_ROOT / "languages" / "binary-multiplication"
MODELS_ROOT = REPO_ROOT / "models" / "binary-multiplication-fixed"
...
def main():
    ...
    out_path = RESULTS / f"fix_trained_models_binary_multiplication_{slug}.json"
    ...
    results = {
        ...
        "experiment": "corrected-dataset-fix (BMF3: single-objective training on corrected data)",
        "description": (
            f"Trains RNN/LSTM/Transformer/Mamba on the corrected binary-multiplication training set "
            f"(languages/binary-multiplication-fixed) under the '{objective}' objective, 10 trials each (40 "
    ...
    for arch in ARCHITECTURES:
        for trial_no in range(1, N_TRIALS + 1):
            if (arch, trial_no) in done_keys:
                continue
            print(f"\n=== training binary-multiplication (corrected) {arch} {objective} trial {trial_no} ===", flush=True)
