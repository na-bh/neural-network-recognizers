"""Evaluates the EXISTING standard-FLaRe-trained repeat-01 models
(data/models/repeat-01/<arch>/<loss_terms>/validation-short/<trial> for
rnn/lstm/transformer, models/repeat-01/mamba/<loss_terms>/validation-
short/<trial> for mamba) on the CORRECTED test set. Pure evaluation, no
training.

PYTHONPATH=src:analysis python analysis/fix_standard_baseline_on_corrected_test_all_objectives_repeat_01.py
"""
...
STANDARD_MODELS_ROOT = REPO_ROOT / "data" / "models" / "repeat-01"
MAMBA_MODELS_ROOT = REPO_ROOT / "models" / "repeat-01"
FIXED_LANG_DIR = REPO_ROOT / "languages" / "repeat-01-fixed"
STANDARD_LANG_DIR = REPO_ROOT / "languages" / "repeat-01"
OUT_PATH = RESULTS / "fix_standard_baseline_on_corrected_test_all_objectives_repeat_01.json"
ARCHITECTURES = ["rnn", "lstm", "transformer", "mamba"]
N_TRIALS = 10


def main():
    results = {"task": "repeat-01",
