"""Evaluates the EXISTING standard-FLaRe-trained binary-addition models
(data/models/binary-addition/<arch>/<loss_terms>/validation-short/<trial>
for rnn/lstm/transformer, models/binary-addition/mamba/<loss_terms>/
validation-short/<trial> for mamba -- all 4 loss terms exist for every
architecture including mamba here, unlike marked-reversal) on the
CORRECTED test set. Pure evaluation, no training.

PYTHONPATH=src:analysis python analysis/fix_standard_baseline_on_corrected_test_all_objectives_binary_addition.py
"""
...
STANDARD_MODELS_ROOT = REPO_ROOT / "data" / "models" / "binary-addition"
MAMBA_MODELS_ROOT = REPO_ROOT / "models" / "binary-addition"
FIXED_LANG_DIR = REPO_ROOT / "languages" / "binary-addition-fixed"
STANDARD_LANG_DIR = REPO_ROOT / "languages" / "binary-addition"
OUT_PATH = RESULTS / "fix_standard_baseline_on_corrected_test_all_objectives_binary_addition.json"
ARCHITECTURES = ["rnn", "lstm", "transformer", "mamba"]
N_TRIALS = 10


def main():
    results = {"task": "binary-addition",
