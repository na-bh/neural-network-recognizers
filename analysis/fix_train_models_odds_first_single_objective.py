"""Corrected-dataset experiment for odds-first, OF3: trains RNN/LSTM/
Transformer/Mamba x 10 seeds (40 runs) under ONE specified loss-term
objective on the corrected training set (languages/odds-first-fixed),
...
objectives_odds_first.py), not embedded here.

PYTHONPATH=src:analysis python analysis/fix_train_models_odds_first_single_objective.py --objective rec
PYTHONPATH=src:analysis python analysis/fix_train_models_odds_first_single_objective.py --objective rec+lm
PYTHONPATH=src:analysis python analysis/fix_train_models_odds_first_single_objective.py --objective rec+ns
PYTHONPATH=src:analysis python analysis/fix_train_models_odds_first_single_objective.py --objective rec+lm+ns
"""
...
STANDARD_LANG_DIR = REPO_ROOT / "languages" / "odds-first"
MODELS_ROOT = REPO_ROOT / "models" / "odds-first-fixed"
TASK = "odds-first"
...
def main():
    ...
    out_path = RESULTS / f"fix_trained_models_odds_first_{slug}.json"
    ...
    results = {
        ...
        "experiment": "corrected-dataset-fix (OF3: single-objective training on corrected data)",
        "description": (
            f"Trains RNN/LSTM/Transformer/Mamba on the corrected odds-first training set "
            f"(languages/odds-first-fixed) under the '{objective}' objective, 10 trials each (40 "
    ...
    for arch in ARCHITECTURES:
        for trial_no in range(1, N_TRIALS + 1):
            if (arch, trial_no) in done_keys:
                continue
            print(f"\n=== training odds-first (corrected) {arch} {objective} trial {trial_no} ===", flush=True)
