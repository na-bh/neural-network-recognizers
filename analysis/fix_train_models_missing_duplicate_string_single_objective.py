"""Corrected-dataset experiment for missing-duplicate-string, MDF3: trains
RNN/LSTM/Transformer/Mamba x 10 seeds (40 runs) under ONE specified loss-
term objective on the corrected training set (languages/missing-duplicate-
string-fixed), matching FLaRe's per-trial hyperparameter sampling exactly
(reuses fix_train_models_all_objectives.py's train_one/evaluate_model_
both_test_sets directly -- no new training-loop logic here). Each run is
evaluated on BOTH the standard FLaRe test set and the corrected test set.
...
objectives_missing_duplicate_string.py), not embedded here.

PYTHONPATH=src:analysis python analysis/fix_train_models_missing_duplicate_string_single_objective.py --objective rec
PYTHONPATH=src:analysis python analysis/fix_train_models_missing_duplicate_string_single_objective.py --objective rec+lm
PYTHONPATH=src:analysis python analysis/fix_train_models_missing_duplicate_string_single_objective.py --objective rec+ns
PYTHONPATH=src:analysis python analysis/fix_train_models_missing_duplicate_string_single_objective.py --objective rec+lm+ns
"""
...
MODELS_ROOT = REPO_ROOT / "models" / "missing-duplicate-string-fixed"
...
def main():
    ...
    out_path = RESULTS / f"fix_trained_models_missing_duplicate_string_{slug}.json"
    ...
    results = {
        ...
        "experiment": "corrected-dataset-fix (MDF3: single-objective training on corrected data)",
        "description": (
            f"Trains RNN/LSTM/Transformer/Mamba on the corrected missing-duplicate-string training "
            f"set (languages/missing-duplicate-string-fixed) under the '{objective}' objective, 10 "
            f"trials each (40 runs). Per-trial hyperparameter sampling matches train_and_evaluate."
            f"bash exactly. Evaluated on both the standard FLaRe test set and the corrected test set."
        ),
    ...
    for arch in ARCHITECTURES:
        for trial_no in range(1, N_TRIALS + 1):
            if (arch, trial_no) in done_keys:
                continue
            print(f"\n=== training missing-duplicate-string (corrected) {arch} {objective} trial {trial_no} ===", flush=True)
