"""Corrected-dataset experiment, BMF1: dataset construction + audit.

Builds a "corrected" binary-multiplication dataset preserving Butoi et al.'s
...
construction: a genuine positive u_x x u_y = u_z (LSB-first), with a SINGLE
bit of u_z flipped (excluding bit 0, the LSB) -- this preserves all five
shortcut features identified in the Phase 3 audit (operator_count_is_one,
equals_count_is_one_after_operator, fields_well_formed, length_sufficient,
lsb_consistent) while making the claimed product genuinely wrong (any
single-bit flip changes the decoded integer value, and bit 0 is excluded
specifically because flipping it always breaks lsb_consistent -- verified
directly in the Phase 3 causal-patching pilot's target_computation_pairs
construction, reused here unchanged at full-dataset scale). Bit-flip
position swept across low/mid/high tertiles with the target held fixed
across retries (Option A discipline, matching compute-sqrt's CSF1).

Reuses recognizers.hand_picked_languages.binary_multiplication.
BinaryMultiplication directly (language.sample() for positives and for the
base string the hard negative is derived from; language.is_negative() to
verify every generated negative really is negative). Unlike compute-sqrt,
compute_z(x,y)=x*y is EXACT integer arithmetic -- no float-precision
divergence to guard against here.
...
standard pipeline):
  languages/binary-multiplication-fixed/{main.tok,labels.txt,
    next-symbols.jsonl,negative-kind.txt,hard-negative-bitflip-meta.jsonl}
  languages/binary-multiplication-fixed/datasets/validation-short/{...}
  languages/binary-multiplication-fixed/datasets/test/{...}

(No log-probabilities.txt: BinaryArithmeticOperation.supports_log_
probability() is False.)

PYTHONPATH=src:analysis python analysis/fix_dataset_binary_multiplication.py
"""
...
LANG_DIR = Path("languages/binary-multiplication-fixed")
...
def _parse(s):
    op_idx = [i for i, t in enumerate(s) if t == OPERATOR]
    eq_idx = [i for i, t in enumerate(s) if t == EQUALS]
    if len(op_idx) != 1:
        return None
    op_i = op_idx[0]
    ...
    if len(eq_idx) != 1 or not later_eq:
        return None
    eq_i = later_eq[0]
    u_x, u_y, u_z = s[:op_i], s[op_i + 1:eq_i], s[eq_i + 1:]
    if not u_x or not u_y or not u_z:
        return None
    return u_x, u_y, u_z


def operator_count_is_one(s):
    return s.count(OPERATOR) == 1


def equals_count_is_one_after_operator(s):
    op_idx = [i for i, t in enumerate(s) if t == OPERATOR]
    eq_idx = [i for i, t in enumerate(s) if t == EQUALS]
...
def fields_well_formed(s):
    return _parse(s) is not None


def length_sufficient(s):
    parts = _parse(s)
    if parts is None:
        return False
    u_x, u_y, u_z = parts
...
def is_genuinely_wrong_product(s):
    u_x, u_y, u_z = _parse(s)
...
def generate_hard_negative_content(language_len_restricted, generator,
    ...
    """Samples a genuine positive u_x x u_y = u_z, flips a single bit of
    u_z (LSB-first, EXCLUDING bit 0 -- flipping the LSB always breaks
    lsb_consistent) at a position drawn from a target tertile held FIXED
    across retries. Field widths untouched -- all five shortcut features
    are structurally guaranteed to survive; z_corrupt != z_clean is
    guaranteed since flipping any single bit changes the decoded integer
    value, and z_clean == x*y exactly (came from a genuine positive, exact
    integer arithmetic), so z_corrupt is guaranteed genuinely wrong."""
    ...
    for _ in range(max_tries):
        ...
        x, y = decode_binary(u_x), decode_binary(u_y)
        z = decode_binary(u_z)
        candidates = list(range(1, n_z))  # exclude bit 0 (the LSB)
        if not candidates:
            continue
        denom = max(1, n_z - 1)
        def frac_of(j):
        ...
        meta = {
            ...
            "bit_position_tertile": target_tertile, "x": x, "y": y, "z_clean": z, "z_corrupt": z_corrupt,
        }
...
def audit_split(rows, split_name):
    ...
    if hard_rows:
        ...
        all_wrong_product = [is_genuinely_wrong_product(r["s"]) for r in hard_rows]
        hard_checks.update({
            "all_pass_all_five_shortcut_features": all(all_pass_shortcuts),
            "n_detectable_by_any_shortcut_feature": sum(1 for x in all_pass_shortcuts if not x),
            "all_genuinely_fail_multiplication_check": all(all_wrong_product),
            "all_constraints_satisfied": all(all_pass_shortcuts) and all(all_wrong_product),
        })
        ...
        hard_checks["magnitude_change_note"] = (
            "magnitude of the value change = 2**bit_idx exactly. Reported via bit_idx rather than "
            "raw magnitude for the same reason as compute-sqrt's CSF1: astronomically large for long "
            "test-set sequences. Same magnitude-scaling setup that produced the magnitude-sensitivity "
            "finding in binary-addition's Phase 3 causal-patching pilot."
        )
    ...
    if uniform_rows:
        ...
        op_counts = Counter(r["s"].count(OPERATOR) for r in uniform_rows)
        eq_counts = Counter(r["s"].count(EQUALS) for r in uniform_rows)
        n_accidentally_valid = sum(
            1 for r in uniform_rows
            if operator_count_is_one(r["s"]) and equals_count_is_one_after_operator(r["s"])
            and fields_well_formed(r["s"]) and not is_genuinely_wrong_product(r["s"])
        )
        uniform_checks.update({
            "operator_count_distribution": {str(k): v for k, v in sorted(op_counts.items())},
            "equals_count_distribution": {str(k): v for k, v in sorted(eq_counts.items())},
            "n_accidentally_correct_product_among_well_formed": n_accidentally_valid,
        })
...
def main():
    ...
    base_lang = BinaryMultiplication()
    ...
    out = {
        ...
        "experiment": "corrected-dataset-fix (BMF1: dataset construction + audit)",
        "description": (
            "Corrected binary-multiplication dataset preserving Butoi et al.'s mixed negative-"
            "generation strategy (50% uniform-random / 50% hard-negative half). Hard negatives are "
            "constructed via a single-bit flip in u_z (LSB-first, excluding bit 0/the LSB) of a "
            "genuine positive -- structurally guaranteed to preserve all five shortcut features "
            "(operator_count_is_one, equals_count_is_one_after_operator, fields_well_formed, "
            "length_sufficient, lsb_consistent), while making the claimed product genuinely wrong. "
            "Bit-flip position swept across low/mid/high tertiles with the target held fixed across "
            "retries (Option A discipline). Reuses the exact target_computation_pairs construction "
            "validated in the Phase 3 causal-patching pilot (analysis/phase3_binmult_counterfactual_"
            "design.py), extended to full-dataset scale."
        ),
        ...
        "known_simplifications_vs_original_pipeline": [
            ...
            "Hard-negative construction rejects (resamples entirely) when n_z < 2 or the target "
            "tertile has no candidate bit position (bit 0 always excluded) for that sample -- minor "
            "documented distributional deviations for the hard-negative half only.",
            "No log-probabilities.txt: BinaryArithmeticOperation.supports_log_probability() is False.",
            "The magnitude-scaling confound (LSB-first encoding means 'high'-tertile bit flips "
            "change the claimed value by an astronomically larger amount than 'low'-tertile flips) is "
            "inherent to the task's own encoding and is NOT corrected by this construction -- reported "
            "(magnitude_change_distribution_by_tertile) for downstream training-time analysis, "
            "matching the same documented, uncorrected confound in binary-addition/compute-sqrt.",
        ],
        "audits": {"train": train_audit, "validation-short": val_audit, "test": test_audit},
    }
    out_path = RESULTS / "fix_dataset_binary_multiplication.json"
