"""Full-sweep per-cell decomposition analysis for bucket-sort, matching the
Dyck-2-3/modular-arithmetic-simple/marked-copy/missing-duplicate-string
template: for each of the 160 corrected-trained cells (10 seeds x 4
architectures x 4 objectives) and each of the 160 standard-FLaRe-trained
cells, re-evaluates on the corrected test set to recover PER-EXAMPLE
correctness, then reports positive/hard-negative/overall accuracy plus
three balanced-discrimination criteria:
  ...
  (c) exceeds_ceiling_balanced: overall_accuracy > THEORETICAL_CEILING
      (from fix_baseline_ceiling_bucket_sort.json) AND balanced_any

Also decomposes hard-negative accuracy by swap-POSITION tertile (low/mid/
high, read DIRECTLY from hard-negative-meta.jsonl's violation_position_
tertile field -- BSF1's construction swept this explicitly).

NOTE on standard-trained cells: unlike marked-copy/missing-duplicate-
string (which have a separate fix_standard_baseline_on_corrected_test_
all_objectives_bucket_sort.json), bucket-sort's per-objective training
scripts embed standard-checkpoint evaluation results directly inside each
fix_trained_models_bucket_sort_<objective>.json's "standard_trained_on_
corrected_test" section -- read from there instead.

PYTHONPATH=src:analysis python analysis/fix_full_sweep_analysis_bucket_sort.py
"""
...
FIXED_LANG_DIR = REPO_ROOT / "languages" / "bucket-sort-fixed"
...
def load_theoretical_ceiling():
    d = json.loads((RESULTS / "fix_baseline_ceiling_bucket_sort.json").read_text())
...
def main():
    ...
    out_path = RESULTS / "fix_full_sweep_analysis_bucket_sort.json"
    ...
    def save():
        ...
        out = {
            ...
            "experiment": (
                ...
                "discrimination, and is any such effect uniform across early/mid/late swap-position "
                "tertiles or concentrated in some?"
            ),
    ...
    # ------------------------------------------------------------------
    for objective in OBJECTIVES:
        run_file = RESULTS / f"fix_trained_models_bucket_sort_{objective.replace('+', '_')}.json"
    ...
    # ------------------------------------------------------------------
    # standard-FLaRe-trained cells -- embedded IN EACH objective's own
    # JSON (bucket-sort-specific convention), not a separate combined file
    # ------------------------------------------------------------------
    for objective in OBJECTIVES:
        run_file = RESULTS / f"fix_trained_models_bucket_sort_{objective.replace('+', '_')}.json"
        if not run_file.exists():
            continue
        d = json.loads(run_file.read_text())
        for r in d.get("standard_trained_on_corrected_test", []):
            if r is None or r.get("eval_failed"):
                continue
            ...
            ev = evaluate_cell(r["model_dir"], arch, seqs, labels, vocab_tokens, pos_idx, uniform_idx, hard_idx, tertile_idx)
            cell = make_cell_record("standard_trained", arch, objective, seed, ev, theoretical_ceiling)
            print(f"  overall={cell['overall_accuracy']:.4f} pos={cell['positive_accuracy']:.4f} "
