"""Corrected-dataset experiment for compute-sqrt, CSF3: trains RNN/LSTM/
Transformer/Mamba x 10 seeds (40 runs) under ONE specified loss-term
objective on the corrected training set (languages/compute-sqrt-fixed),
...
objectives_compute_sqrt.py), not embedded here.

PYTHONPATH=src:analysis python analysis/fix_train_models_compute_sqrt_single_objective.py --objective rec
PYTHONPATH=src:analysis python analysis/fix_train_models_compute_sqrt_single_objective.py --objective rec+lm
PYTHONPATH=src:analysis python analysis/fix_train_models_compute_sqrt_single_objective.py --objective rec+ns
PYTHONPATH=src:analysis python analysis/fix_train_models_compute_sqrt_single_objective.py --objective rec+lm+ns
"""
...
STANDARD_LANG_DIR = REPO_ROOT / "languages" / "compute-sqrt"
MODELS_ROOT = REPO_ROOT / "models" / "compute-sqrt-fixed"
TASK = "compute-sqrt"
...
def main():
    ...
    out_path = RESULTS / f"fix_trained_models_compute_sqrt_{slug}.json"
    ...
    results = {
        ...
        "experiment": "corrected-dataset-fix (CSF3: single-objective training on corrected data)",
        "description": (
            f"Trains RNN/LSTM/Transformer/Mamba on the corrected compute-sqrt training set "
            f"(languages/compute-sqrt-fixed) under the '{objective}' objective, 10 trials each (40 "
    ...
    for arch in ARCHITECTURES:
        for trial_no in range(1, N_TRIALS + 1):
            if (arch, trial_no) in done_keys:
                continue
            print(f"\n=== training compute-sqrt (corrected) {arch} {objective} trial {trial_no} ===", flush=True)
