"""Shared causal-patching runner functions for binary-multiplication's
bounded Phase 3 pilot -- task-parameterized versions of the RNN/LSTM/
Mamba/Transformer runner boilerplate already established and reused across
every prior task in this pilot.

Uses separate clean/corrupt readout positions throughout (Condition 2's
length_insufficient pairs are NOT same-length).

Checkpoints: rec+ns/validation-short/<seed>, matching every other task's
Phase 3 patching in this pilot.

PYTHONPATH=src:analysis python analysis/phase3_binmult_patching_common.py
"""
...
CELLS = [
    ("rnn", 4, "primary"), ("rnn", 2, "secondary"),
    ("lstm", 8, "primary"), ("lstm", 2, "secondary"),
    ("transformer", 9, "primary"), ("transformer", 4, "secondary"),
    ("mamba", 4, "primary"), ("mamba", 8, "secondary"),
]
...
def load_cell_selection_context():
    d = json.loads((RESULTS / "phase3_binmult_cell_selection.json").read_text())
