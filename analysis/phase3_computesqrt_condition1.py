"""Compute-sqrt Phase 3, condition 1 of 3: structural features
(marker_count EARLY, no_marker, marker_position, null_control TRUE MINIMAL
PAIR).
...
Saves analysis_outputs/final_results/phase3_computesqrt_condition1.json.
STOP after this condition.

PYTHONPATH=src:analysis python analysis/phase3_computesqrt_condition1.py
"""
...
from phase3_computesqrt_counterfactual_design import (
    marker_count_pairs, no_marker_pairs, marker_position_pairs, null_control_pairs,
    audit_marker_count_pairs, audit_no_marker_pairs, audit_marker_position_pairs, audit_null_control_pairs,
)
...
def main():
    ...
    nmk = no_marker_pairs(N_PAIRS, rng)
    mp = marker_position_pairs(N_PAIRS, rng)
    ...
    for arch, seed, role in CELLS:
        ...
        for pair_type, pairs in [
            ("marker_count_EARLY", mc), ("no_marker", nmk), ("marker_position", mp),
            ("null_control_TRUE_MINIMAL_PAIR", nc),
        ]:
    ...
    out = {
        ...
        "status": "PRIMARY evidence is the behavioral logit gap (clean vs corrupt, unpatched); "
                 ...
                 "near zero (< 0.05).",
        "description": "Tests whether marker_count (EARLY placement), no_marker (well-formedness), "
                       "and marker_position causally drive the accept/reject decision, all 8 cells. "
                       "null_control is a TRUE 1-position minimal pair exploiting compute-sqrt's own "
                       "many-to-one x->z structure -- both members genuinely valid, same true label; "
                       "expected near-zero gap everywhere.",
        "n_pairs_generated": N_PAIRS,
        "pair_construction_audits": audits,
        "sample_pairs": {"marker_count": mc[:3], "no_marker": nmk[:3], "marker_position": mp[:3], "null_control": nc[:3]},
        "cells": cells,
        "null_control_check": null_control_check,
    }
    out_path = RESULTS / "phase3_computesqrt_condition1.json"
