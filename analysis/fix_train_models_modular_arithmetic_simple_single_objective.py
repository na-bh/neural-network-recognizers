"""Corrected-dataset experiment for modular-arithmetic-simple, MF3: trains
RNN/LSTM/Transformer/Mamba x 10 seeds (40 runs) under ONE specified loss-term
objective on the corrected training set (languages/modular-arithmetic-
simple-fixed), matching FLaRe's per-trial hyperparameter sampling exactly
(reuses fix_train_models_all_objectives.py's train_one/evaluate_model_both_
test_sets directly -- no new training-loop logic here).

Also evaluates the EXISTING standard-FLaRe-trained checkpoints for the SAME
objective (data/models/modular-arithmetic-simple/<arch>/<objective>/
validation-short/<1..10>, or models/modular-arithmetic-simple/mamba/... for
mamba's 0-9 seed range) on the corrected test set -- no retraining, pure
evaluation, included in the same output file for direct comparison.
...
PYTHONPATH=src:analysis python analysis/fix_train_models_modular_arithmetic_simple_single_objective.py --objective rec
PYTHONPATH=src:analysis python analysis/fix_train_models_modular_arithmetic_simple_single_objective.py --objective rec+lm
PYTHONPATH=src:analysis python analysis/fix_train_models_modular_arithmetic_simple_single_objective.py --objective rec+ns
PYTHONPATH=src:analysis python analysis/fix_train_models_modular_arithmetic_simple_single_objective.py --objective rec+lm+ns
"""
...
STANDARD_LANG_DIR = REPO_ROOT / "languages" / "modular-arithmetic-simple"
STANDARD_MODELS_ROOT = REPO_ROOT / "data" / "models" / "modular-arithmetic-simple"
MAMBA_STANDARD_MODELS_ROOT = REPO_ROOT / "models" / "modular-arithmetic-simple"
MODELS_ROOT = REPO_ROOT / "models" / "modular-arithmetic-simple-fixed"
...
def main():
    ...
    out_path = RESULTS / f"fix_trained_models_modular_arithmetic_simple_{slug}.json"
    ...
    results = {
        ...
        "experiment": "corrected-dataset-fix (MF3: single-objective training on corrected data)",
        "description": (
            f"Trains RNN/LSTM/Transformer/Mamba on the corrected modular-arithmetic-simple "
            f"training set (languages/modular-arithmetic-simple-fixed) under the '{objective}' "
            f"objective, 10 trials each (40 runs). Per-trial hyperparameter sampling matches "
            f"train_and_evaluate.bash exactly. Evaluated on both the standard FLaRe test set and "
            f"the corrected test set. Also includes the EXISTING standard-FLaRe-trained "
            f"checkpoints for this same objective, evaluated on the corrected test set (no "
            f"retraining), for direct comparison."
        ),
    ...
    for arch in ARCHITECTURES:
        for trial_no in range(1, N_TRIALS + 1):
            if (arch, trial_no) in done_keys:
                continue
            print(f"\n=== training modular-arithmetic-simple (corrected) {arch} {objective} trial {trial_no} ===", flush=True)
