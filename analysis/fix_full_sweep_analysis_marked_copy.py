"""Full-sweep per-cell decomposition analysis for marked-copy, matching the
...
overall accuracy plus three balanced-discrimination criteria:
  ...
  (c) exceeds_ceiling_balanced: overall_accuracy > THEORETICAL_CEILING
      (from fix_baseline_ceiling_marked_copy.json) AND balanced_any
...
marked-copy's swap positions i,j are drawn i uniformly then j as any
differing position, NOT an adjacent pair swept by tertile the way missing-
duplicate-string's construction was, so the tertile bucket here is derived
post-hoc from the ANCHOR position i, not a pre-recorded discrete label) --
checking whether the fix's effect (where present) is uniform across
early/mid/late anchor positions or concentrated in some.

PYTHONPATH=src:analysis python analysis/fix_full_sweep_analysis_marked_copy.py
"""
...
def load_theoretical_ceiling():
    d = json.loads((RESULTS / "fix_baseline_ceiling_marked_copy.json").read_text())
...
def main():
    ...
    out_path = RESULTS / "fix_full_sweep_analysis_marked_copy.json"
    ...
    # ------------------------------------------------------------------
    for objective in OBJECTIVES:
        run_file = RESULTS / f"fix_trained_models_marked_copy_{objective.replace('+', '_')}.json"
    ...
    # ------------------------------------------------------------------
    std_file = RESULTS / "fix_standard_baseline_on_corrected_test_all_objectives_marked_copy.json"
    if std_file.exists():
