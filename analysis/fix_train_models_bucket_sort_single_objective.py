"""Corrected-dataset experiment for bucket-sort, BSF3: trains RNN/LSTM/
Transformer/Mamba x 10 seeds (40 runs) under ONE specified loss-term
objective on the corrected training set (languages/bucket-sort-fixed),
...
objective (data/models/bucket-sort/<arch>/<objective>/validation-short/
<1..10>, models/bucket-sort/mamba/<objective>/validation-short/<0..9>) on
the corrected test set -- no retraining, pure evaluation, included in the
same output file for direct comparison.

PYTHONPATH=src:analysis python analysis/fix_train_models_bucket_sort_single_objective.py --objective rec
PYTHONPATH=src:analysis python analysis/fix_train_models_bucket_sort_single_objective.py --objective rec+lm
PYTHONPATH=src:analysis python analysis/fix_train_models_bucket_sort_single_objective.py --objective rec+ns
PYTHONPATH=src:analysis python analysis/fix_train_models_bucket_sort_single_objective.py --objective rec+lm+ns
"""
...
STANDARD_LANG_DIR = REPO_ROOT / "languages" / "bucket-sort"
STANDARD_MODELS_ROOT = REPO_ROOT / "data" / "models" / "bucket-sort"
MAMBA_STANDARD_MODELS_ROOT = REPO_ROOT / "models" / "bucket-sort"
MODELS_ROOT = REPO_ROOT / "models" / "bucket-sort-fixed"
TASK = "bucket-sort"
...
def main():
    ...
    out_path = RESULTS / f"fix_trained_models_bucket_sort_{slug}.json"
    ...
    results = {
        ...
        "experiment": "corrected-dataset-fix (BSF3: single-objective training on corrected data)",
        "description": (
            f"Trains RNN/LSTM/Transformer/Mamba on the corrected bucket-sort training set "
            f"(languages/bucket-sort-fixed) under the '{objective}' objective, 10 trials each (40 "
    ...
    for arch in ARCHITECTURES:
        for trial_no in range(1, N_TRIALS + 1):
            if (arch, trial_no) in done_keys:
                continue
            print(f"\n=== training bucket-sort (corrected) {arch} {objective} trial {trial_no} ===", flush=True)
