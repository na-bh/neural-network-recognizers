"""Shared causal-patching runner functions for Dyck-2-3's bounded Phase 3
pilot -- task-parameterized versions of the RNN/LSTM/Mamba/Transformer
runner boilerplate already established and reused across every prior task
in this pilot (bucket-sort, binary-addition, etc.).

PYTHONPATH=src:analysis python analysis/phase3_dyck_patching_common.py
"""
...
CELLS = [
    ("rnn", 9, "primary"), ("rnn", 8, "secondary"),
    ("lstm", 2, "primary"), ("lstm", 7, "secondary"),
    ("transformer", 9, "primary"), ("transformer", 10, "secondary"),
    ("mamba", 0, "primary"), ("mamba", 8, "secondary"),
]
...
def run_rnn_lstm(arch, seed, pairs, tok2idx):
    ...
    for p in pairs:
        ...
        corrupt_idx = [tok2idx[t] for t in p["corrupted"]]
        t_ro_clean = h.readout_position(p["clean"])
        t_ro_corrupt = h.readout_position(p["corrupted"])
    ...
    return results, f"models/{TASK}/{arch}/rec+ns/validation-short/{seed}"
...
def run_transformer(seed, pairs, tok2idx):
    ...
    for p in pairs:
        ...
        t_ro_corrupt = len(p["corrupted"]) - 1
        lc = h.logit_diff(clean_idx)
        lo = h.logit_diff(corrupt_idx)
        _, clean_cache = h.run_with_cache(clean_idx, layers=[layer])
        ...
        results.append({"clean_logit": lc, "corrupt_logit": lo, "patched_logit": patched,
                        "restored_fraction": rf, "behavioral_gap": abs(lc - lo)})
    return results, f"models/{TASK}/transformer/rec+ns/validation-short/{seed}"
...
# per-seed chance-level context (from phase3_dyck_cell_selection.json), used
# to distinguish "null because it doesn't solve the task" from "null because
# it uses different features" -- specifically for Transformer, per this
# condition's approval.
def load_cell_selection_context():
    d = json.loads((RESULTS / "phase3_dyck_cell_selection.json").read_text())
