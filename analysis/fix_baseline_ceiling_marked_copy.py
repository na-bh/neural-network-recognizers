"""Corrected-dataset experiment, MCF2: shortcut-only baseline ceiling on
the corrected test set.

Marked-copy has the SAME marker structure as marked-reversal (single '#'
separating two equal-length halves) -- only the content transform differs
(copy vs reversal), which the marker_shortcut_features feature set
(analysis/phase1_b3_compare.py: marker count, marker relative offset from
center) never encodes at all. Reused directly, unchanged, from marked-
reversal's F2. No pre-existing phase1 checkpoints exist for marked-copy
(unlike marked-reversal), so all three baselines (shortcut, ngram-logreg,
bagcount-ffn) are TRAINED HERE on marked-copy's STANDARD training data --
matching bucket-sort's BSF2 convention, not marked-reversal's F2 (which
reused pre-existing checkpoints).

PYTHONPATH=src:analysis python analysis/fix_baseline_ceiling_marked_copy.py
"""
...
TASK = "marked-copy"
FIXED_TEST_DIR = Path("languages/marked-copy-fixed/datasets/test")
LONG_SEQ_THRESH = 400
H_GRID = [8, 32, 128, 512]
MARKER = "#"
...
def provable_incapacity_counterexample():
    ...
    opposite validity -- the swap breaks copy-content while leaving marker
    structure untouched."""
    clean = ["0", "1", "1", "#", "0", "1", "1"]  # w=011, w#w -- valid
    corrupted = ["0", "1", "1", "#", "0", "1", "0"]  # last position flipped -- invalid copy
    ...
    return {
        ...
        "note": "IDENTICAL marker-shortcut feature vectors (marker count/relative-offset), "
               ...
               "marker half is actually a COPY, not just correctly bracketed) is structurally "
               "absent from a marker-position-only representation.",
    }
...
def main():
    ...
    ceiling_report = {
        ...
        "formula": "(n_positive + n_structurally_obvious + 0.5 * n_structurally_ambiguous) / n_total "
                  ...
                  "alone) -- i.e. perfect on the uniform-random half (essentially never single-"
                  "marker-and-balanced by chance) + chance on the hard-negative half (single-"
                  "marker-and-balanced BY CONSTRUCTION).",
        ...
        "note": (
            ...
            f"structurally ambiguous purely by chance (expected small/near-zero, checked directly)."
        ),
    }
    print("\n=== theoretical shortcut-only ceiling ===")
    ...
    # ------------------------------------------------------------------
    print("\n=== training marker_shortcut baseline on standard marked-copy training data ===", flush=True)
    ...
    ms_ckpt_path = CKPT_DIR / "phase1_marked_copy_marker_shortcut.joblib"
    ...
    ms_result["trained_on"] = "standard marked-copy training set"
    ...
    # ------------------------------------------------------------------
    print("\n=== training ngram-logreg on standard marked-copy training data ===", flush=True)
    with warnings.catch_warnings():
        warnings.simplefilter("ignore")
    ...
    ng_ckpt_path = CKPT_DIR / "phase1_marked_copy_ngram_logreg.joblib"
    ...
    ng_result["trained_on"] = "standard marked-copy training set"
    ...
    bagcount_results = {}
    for h in H_GRID:
        print(f"\n=== training bagcount-ffn h={h} on standard marked-copy training data ===", flush=True)
        tB = train_candidate_B(TASK, h, vocab, train_seqs, train_labels, val_seqs, val_labels)
        ckpt_path = CKPT_DIR / f"phase1_marked_copy_bagcount_ffn_h{h}.pt"
        ...
        bc_result["trained_on"] = "standard marked-copy training set"
    ...
    out = {
        "task": "marked-copy",
        "experiment": "corrected-dataset-fix (MCF2: shortcut-only baseline ceiling on corrected test data)",
        "description": (
            "Evaluates a NEWLY-TRAINED marker_shortcut baseline (marker count/relative-position, "
            "reused unchanged from marked-reversal's feature set -- provably incapable of copy-"
            "content verification) plus ngram-logreg and bagcount-ffn baselines (also newly "
            "trained here, no pre-existing phase1 checkpoints for marked-copy), all trained on "
            "STANDARD marked-copy training data, evaluated on the CORRECTED test set (MCF1), split "
    ...
    out_path = RESULTS / "fix_baseline_ceiling_marked_copy.json"
