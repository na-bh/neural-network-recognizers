"""Repeat-01 Phase 3 (bounded) pilot: cell selection. Pulls trained-
architecture accuracies from flare_a2_repeat_01.json (existing checkpoints,
no training here -- the file already exists), selects primary (best) +
secondary (next-best) seed per architecture, and checks for single-solver
bottlenecks (matching phase3_dyck/missdup/stackmanip/computesqrt/
binmult_cell_selection.py's discipline exactly).

PYTHONPATH=src:analysis python analysis/phase3_repeat01_cell_selection.py
"""
...
def select_cells():
    flare_a2 = json.loads((RESULTS / "flare_a2_repeat_01.json").read_text())
...
def main():
    ...
    print("\n=== hard-vs-scrambled accuracy (reported for context -- interpret per the task audit's "
          "viol-vs-shortcut-blind divergence finding) ===", flush=True)
    print(json.dumps(hard_vs_scrambled, indent=2, default=str))

    out = {
        "task": TASK,
        "description": (
            "Cell selection for repeat-01's bounded Phase 3 patching pilot: primary (best) + "
            "secondary (next-best) seed per architecture, pulled from the existing flare_a2_"
            "repeat_01.json (no new training). Checks for per-seed single-solver bottlenecks AND "
        ...
        "hard_vs_scrambled_negative_accuracy_interpret_with_task_audit_caveat": hard_vs_scrambled,
        "final_cells": [
    ...
    out_path = RESULTS / "phase3_repeat01_cell_selection.json"
