"""Full-sweep per-cell decomposition analysis for repeat-01, matching the
...
overall accuracy plus three balanced-discrimination criteria:
  ...
  (c) exceeds_ceiling_balanced: overall_accuracy > THEORETICAL_CEILING
      (from fix_baseline_ceiling_repeat_01.json, 0.8750) AND balanced_any

IMPORTANT FRAMING NOTE (per explicit user instruction): the comparison
threshold for "does this model exceed a shortcut-only detector" is the
THEORETICAL shortcut-only ceiling (0.8750), NOT the ngram-logreg baseline's
measured accuracy. RF2 found that ngram-logreg is NOT a legitimate shortcut-
only baseline for repeat-01 -- alternation validity in (01)* is a purely
local bigram constraint (no "00"/"11" adjacent pair is ever valid), so a
bag-of-bigrams classifier can solve the TRUE target exactly (it measured
1.0000 overall / 1.0000 hard-negative, exceeding the theoretical ceiling
itself). ngram-logreg's accuracy is therefore NOT a valid upper bound for
"shortcut-only" performance on this task and must not be used as the bar a
trained recognizer needs to "beat" -- exceeds_ceiling_balanced compares
against the four-feature (length_parity_even, first_symbol_is_0,
last_symbol_is_1, count_balance) theoretical ceiling only.

Also decomposes hard-negative accuracy by swap-position tertile (low/mid/
high), using the PRE-RECORDED swap_position_tertile label from RF1's
hard-negative-meta.jsonl directly (RF1's construction explicitly swept the
target tertile per example and held it fixed across retries -- Option A
discipline -- so no post-hoc bucketing is needed here, unlike odds-first).

PYTHONPATH=src:analysis python analysis/fix_full_sweep_analysis_repeat_01.py
"""
...
def load_theoretical_ceiling():
    d = json.loads((RESULTS / "fix_baseline_ceiling_repeat_01.json").read_text())
...
def main():
    ...
    out_path = RESULTS / "fix_full_sweep_analysis_repeat_01.json"
    ...
    def save():
        crit = {
            ...
            "c_exceeds_ceiling_balanced": f"overall_accuracy > {theoretical_ceiling:.4f} (theoretical "
                                          f"shortcut-only ceiling, NOT the ngram-logreg baseline -- see "
                                          f"module docstring) AND balanced_any",
        }
        out = {
            "task": TASK,
            "experiment": (
                ...
                "or concentrated in some?"
            ),
            "important_ngram_baseline_note": (
                "ngram-logreg measured 1.0000 overall / 1.0000 hard-negative accuracy on this task "
                "(fix_baseline_ceiling_repeat_01.json) -- it EXCEEDS the theoretical shortcut-only "
                "ceiling because alternation validity is a purely local bigram constraint, fully "
                "solvable by a bag-of-bigrams model. It is therefore NOT a valid shortcut-only "
                "baseline and is NOT used as the exceeds_ceiling_balanced comparison threshold. The "
                "threshold is the theoretical ceiling computed from the four structural features "
                "(length_parity_even, first_symbol_is_0, last_symbol_is_1, count_balance) only."
            ),
    ...
    # ------------------------------------------------------------------
    for objective in OBJECTIVES:
        run_file = RESULTS / f"fix_trained_models_repeat_01_{objective.replace('+', '_')}.json"
    ...
    # ------------------------------------------------------------------
    std_file = RESULTS / "fix_standard_baseline_on_corrected_test_all_objectives_repeat_01.json"
    if std_file.exists():
