"""Shared causal-patching runner functions for missing-duplicate-string's
bounded Phase 3 pilot -- task-parameterized versions of the RNN/LSTM/Mamba/
Transformer runner boilerplate already established and reused across every
prior task in this pilot (dyck-2-3, bucket-sort, etc.).

Uses separate clean/corrupt readout positions throughout (not assumed
equal) since Condition 1's length_parity pairs have clean/corrupt of
DIFFERENT lengths -- matches phase3_dyck_patching_common.py's general
pattern, not bucket-sort's condition1.py's same-length-only shortcut.

Checkpoints: rec+ns/validation-short/<seed>, matching every other task's
Phase 3 patching in this pilot (data/models/<task>/<arch>/... for rnn/lstm/
transformer, models/<task>/mamba/... for mamba).

PYTHONPATH=src:analysis python analysis/phase3_missdup_patching_common.py
"""
...
CELLS = [
    ("rnn", 2, "primary"), ("rnn", 3, "secondary"),
    ("lstm", 1, "primary"), ("lstm", 2, "secondary"),
    ("transformer", 8, "primary"), ("transformer", 10, "secondary"),
    ("mamba", 9, "primary"), ("mamba", 5, "secondary"),
]
...
def load_cell_selection_context():
    d = json.loads((RESULTS / "phase3_missdup_cell_selection.json").read_text())
    return d["cell_selection"], d["architecture_wide_route_hint"]
