"""Evaluates the EXISTING standard-FLaRe-trained odds-first models
(data/models/odds-first/<arch>/<loss_terms>/validation-short/<trial> for
rnn/lstm/transformer, models/odds-first/mamba/<loss_terms>/validation-
short/<trial> for mamba) on the CORRECTED test set. Pure evaluation, no
training.

PYTHONPATH=src:analysis python analysis/fix_standard_baseline_on_corrected_test_all_objectives_odds_first.py
"""
...
STANDARD_MODELS_ROOT = REPO_ROOT / "data" / "models" / "odds-first"
MAMBA_MODELS_ROOT = REPO_ROOT / "models" / "odds-first"
FIXED_LANG_DIR = REPO_ROOT / "languages" / "odds-first-fixed"
STANDARD_LANG_DIR = REPO_ROOT / "languages" / "odds-first"
OUT_PATH = RESULTS / "fix_standard_baseline_on_corrected_test_all_objectives_odds_first.json"
ARCHITECTURES = ["rnn", "lstm", "transformer", "mamba"]
N_TRIALS = 10


def main():
    results = {"task": "odds-first",
