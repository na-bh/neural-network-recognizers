"""Compute-sqrt Phase 3, condition 3 of 3: target-computation null test
(the load-bearing test) on the value_mismatch_only residual. clean/corrupt
pairs hold all three shortcut features fixed (single marker, well-formed
fields, sufficient answer length -- structurally guaranteed since only one
u_z bit is touched) and differ ONLY in one flipped bit of z, producing a
wrong value. Swept LOW/MID/HIGH tertile by flipped-bit index within u_z --
since encoding is LSB-first (confirmed in the task audit), LOW-tertile
flips are LOW-ORDER (small-magnitude) and HIGH-tertile flips are HIGH-
ORDER (large-magnitude). N=25 PER TERTILE per cell (75 pairs per cell, 8
cells).

BEFORE interpreting the causal (patching) results, this ALSO runs a direct-
accuracy check (bypassing patching entirely) on the exact 411-example
value_mismatch_only shortcut-blind population from the REAL test set
(phase3_computesqrt_task_audit.json's joint residual) for each of the 8
cells. Per the user's explicit three-way framing:
  READING 1 -- substantially-above-chance real-subset accuracy AND
    POSITION-INDEPENDENT causal Condition 3 gaps: genuine bit-exact
    squaring-verification (would be the FIRST such finding in the pilot).
  READING 2 (magnitude-scaling) -- gaps scale with tertile (high > low):
    binary-addition-style magnitude sensitivity, NOT comprehensive
    verification -- extends that finding to a second arithmetic task.
  READING 3 (floor effect) -- near-constant real-subset accuracy (floor)
    AND near-zero Condition 3 gaps across all tertiles: floor-effect
    pattern matching missing-duplicate-string/stack-manipulation --
    compute-sqrt joins the confounded-hard-categorization cluster.
Any cell scoring unusually high (especially ~100%) on the all-negative
411-example subset is checked against a REAL LONG-POSITIVE control before
being accepted as genuine verification (matching the stack-manipulation
Transformer catch).
...
Saves analysis_outputs/final_results/phase3_computesqrt_condition3.json.

PYTHONPATH=src:analysis python analysis/phase3_computesqrt_condition3.py
"""
...
from phase3_computesqrt_counterfactual_design import (
    target_computation_pairs, audit_target_computation_pairs, TERTILES, SHORTCUT_FEATURES, decode_binary,
)
from phase3_computesqrt_patching_common import CELLS, get_tok2idx, run_cell, summarize
from phase3_computesqrt_task_audit import classify_violation, load_split
...
TASK = "compute-sqrt"
TASK_DIR = Path("languages/compute-sqrt")
...
MAGNITUDE_SCALING_RATIO = 2.0  # high-tertile gap must exceed low-tertile gap by this factor

CHECKPOINT_DIRS = {
    ("rnn", 1): Path("data/models/compute-sqrt/rnn/rec+ns/validation-short/1"),
    ("rnn", 8): Path("data/models/compute-sqrt/rnn/rec+ns/validation-short/8"),
    ("lstm", 6): Path("data/models/compute-sqrt/lstm/rec+ns/validation-short/6"),
    ("lstm", 7): Path("data/models/compute-sqrt/lstm/rec+ns/validation-short/7"),
    ("transformer", 7): Path("data/models/compute-sqrt/transformer/rec+ns/validation-short/7"),
    ("transformer", 2): Path("data/models/compute-sqrt/transformer/rec+ns/validation-short/2"),
    ("mamba", 0): Path("models/compute-sqrt/mamba/rec+ns/validation-short/0"),
    ("mamba", 5): Path("models/compute-sqrt/mamba/rec+ns/validation-short/5"),
}
...
def get_long_positive_control():
    seqs, labels = load_split("test")
    lens = sorted(len(s) for s, l in zip(seqs, labels) if l == 0 and
                  all(fn(s) for fn in SHORTCUT_FEATURES.values()))
    threshold = lens[0] if lens else 30
    return [(s, l) for s, l in zip(seqs, labels) if l == 1 and len(s) >= threshold]
...
def load_model(arch, model_dir):
    ...
    tmp = tempfile.mkdtemp(prefix="computesqrt_condition3_")
...
def classify_reading(acc, gaps_by_tertile):
    ...
    mean_gap = float(np.mean([low, mid, high]))
    ...
    if above_chance and position_independent:
        return "READING_1_genuine_bit_exact_verification"
    if magnitude_scaling:
        return "READING_2_magnitude_scaling_like_binary_addition"
    if floor and not any_causal:
        return "READING_3_floor_effect_confounded_hard_cluster"
    return "UNCLASSIFIED"
...
def main():
    ...
    # ------------------------------------------------------------------
    # direct-accuracy check on the 411-example REAL value_mismatch_only
    ...
    # (>=0.90) accuracy on the all-negative hard subset -- matching the
    # stack-manipulation Transformer catch
    # ------------------------------------------------------------------
    long_pos_control = get_long_positive_control()
    print(f"\n=== long-positive control check (n={len(long_pos_control)}) for any cell with "
          f">=0.90 accuracy on the genuinely-hard subset ===", flush=True)
    control_checks = {}
    ...
    out = {
        ...
        "status": "PRIMARY evidence is the behavioral logit gap (clean vs corrupt, unpatched); "
                 ...
                 "near zero (< 0.05).",
        "description": (
            "Tests genuine bit-exact value-verification: clean/corrupt pairs hold all three "
            "shortcut features fixed (marker count, well-formedness, answer-length-sufficiency), "
            "differing ONLY via a single flipped bit in u_z. Swept LOW/MID/HIGH tertile by bit "
            "index (LSB-first: low=small-magnitude, high=large-magnitude), N=25 per tertile per "
            "cell. ALSO runs a direct-accuracy check on the 411-example real value_mismatch_only "
            "shortcut-blind population, and a long-positive control for any cell scoring >=0.90 on "
            "that all-negative subset (matching the stack-manipulation Transformer catch)."
        ),
    ...
    out_path = RESULTS / "phase3_computesqrt_condition3.json"
