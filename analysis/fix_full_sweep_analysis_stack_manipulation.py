"""Full-sweep per-cell decomposition analysis for stack-manipulation, matching the
...
overall accuracy plus three balanced-discrimination criteria:
  ...
  (c) exceeds_ceiling_balanced: overall_accuracy > THEORETICAL_CEILING
      (from fix_baseline_ceiling_stack_manipulation.json, 0.8750) AND balanced_any

IMPORTANT FRAMING NOTE (per explicit user instruction): SMF2 found two
DISTINCT bagcount-ffn failure modes at different hidden widths, not a
smooth capacity trend -- h8 (plus shortcut/ngram) collapses to ACCEPT-bias
(hard_negative_accuracy=0.00, positive_accuracy high), while h32/h128/h512
flip to near-total REJECT-bias (hard_negative_accuracy~0.99, but
positive_accuracy near 0, overall pinned at ~0.50). Neither extreme is
genuine discrimination -- balanced_any (positive_accuracy > 0.5 AND
hard_negative_accuracy > 0.5 SIMULTANEOUSLY) is the only criterion that
counts as real evidence a cell escaped both failure modes; exceeding one
bagcount extreme in isolation (e.g. matching or beating h32's 0.99 hard-
negative accuracy while positive_accuracy stays near 0) is NOT progress
and must not be reported as such.

Also decomposes hard-negative accuracy by swap-position tertile (low/mid/
high), using the PRE-RECORDED swap_position_tertile label from SMF1's
hard-negative-swap-meta.jsonl directly (SMF1's construction explicitly
swept the target tertile per example and held it fixed across retries --
Option A discipline -- so no post-hoc bucketing is needed here, unlike
odds-first). Phase 3 causal patching found 6/8 cells at floor-effect on a
62-example genuinely-hard subset; this corrected test set's ~1260
genuinely-hard examples give far more statistical power to detect whether
corrected training pushes any cell past floor at all, in any tertile.

PYTHONPATH=src:analysis python analysis/fix_full_sweep_analysis_stack_manipulation.py
"""
...
FIXED_LANG_DIR = REPO_ROOT / "languages" / "stack-manipulation-fixed"
...
def load_theoretical_ceiling():
    d = json.loads((RESULTS / "fix_baseline_ceiling_stack_manipulation.json").read_text())
...
def load_fixed_test():
    ...
    with (FIXED_TEST_DIR / "hard-negative-swap-meta.jsonl").open() as f:
        for line in f:
            d = json.loads(line)
            tertiles.append(d.get("swap_position_tertile"))
...
def main():
    ...
    out_path = RESULTS / "fix_full_sweep_analysis_stack_manipulation.json"
    ...
    def save():
        crit = {
            ...
            "c_exceeds_ceiling_balanced": f"overall_accuracy > {theoretical_ceiling:.4f} (theoretical "
                                          f"shortcut-only ceiling) AND balanced_any",
        }
        out = {
            "task": TASK,
            "experiment": (
                ...
                "discrimination, and is any such effect uniform across early/mid/late swap positions "
                "or concentrated in some? Specifically: does any cell escape the two distinct SMF2 "
                "bagcount-ffn failure modes (h8-style accept-bias, h32-style reject-bias) into "
                "genuine balanced_any territory?"
            ),
            "important_bagcount_failure_mode_note": (
                "SMF2 found two DISTINCT failure modes for the shortcut-only bagcount-ffn baseline, "
                "not a smooth capacity trend: h8 (plus shortcut/ngram) collapses to ACCEPT-bias "
                "(hard_negative_accuracy=0.00), h32/h128/h512 flip to near-total REJECT-bias "
                "(hard_negative_accuracy~0.99, positive_accuracy near 0, overall pinned at ~0.50). "
                "Neither is genuine discrimination. balanced_any (BOTH positive_accuracy > 0.5 AND "
                "hard_negative_accuracy > 0.5) is the only criterion that counts -- a corrected-"
                "trained cell matching or exceeding h32's 0.99 hard-negative accuracy while its "
                "positive_accuracy stays near 0 is just landing in the same reject-bias failure mode, "
    ...
    # ------------------------------------------------------------------
    for objective in OBJECTIVES:
        run_file = RESULTS / f"fix_trained_models_stack_manipulation_{objective.replace('+', '_')}.json"
    ...
    # ------------------------------------------------------------------
    std_file = RESULTS / "fix_standard_baseline_on_corrected_test_all_objectives_stack_manipulation.json"
    if std_file.exists():
