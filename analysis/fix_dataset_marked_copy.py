"""Corrected-dataset experiment, MCF1: dataset construction + audit.

Builds a "corrected" marked-copy dataset preserving Butoi et al.'s mixed
negative-generation strategy (50% uniform-random / 50% "adversarial" half)
but replacing the K-edit-perturbation adversarial half with an explicit
hard-negative construction: a genuine positive w#w, with two positions in
the post-marker half swapped -- this preserves marker count (1), marker
...
literal copy-content match. SAME construction template as marked-reversal's
F1 (analysis/fix_dataset_marked_reversal_f1.py), adapted for COPY instead
of REVERSAL -- the only semantic difference is is_genuinely_mismatched
compares against the un-reversed pre-marker half.

Reuses recognizers.hand_picked_languages.marked_copy.MarkedCopy directly
...
really is negative, matching Butoi et al.'s own rejection-sampling
discipline).
...
standard pipeline):
  languages/marked-copy-fixed/{main.tok,labels.txt,next-symbols.jsonl,
    log-probabilities.txt,negative-kind.txt,hard-negative-swap-meta.jsonl}
  languages/marked-copy-fixed/datasets/validation-short/{...}
  languages/marked-copy-fixed/datasets/test/{...}

PYTHONPATH=src:analysis python analysis/fix_dataset_marked_copy.py
"""
...
LANG_DIR = Path("languages/marked-copy-fixed")
...
def generate_hard_negative_content(language_len_restricted, marker_symbol, generator,
    ...
    """Samples a genuine positive w#w, then swaps two positions within the
    post-marker (copy) half whose VALUES differ -- this breaks the literal
    copy-content match while preserving marker count/position, balanced
    halves, length, and the post-marker half's content multiset (a swap can
    never change a multiset). Position i is chosen uniformly at random
    within the half each call, so aggregated across many generated examples
    the mismatch position is NOT concentrated at any single index (the
    Phase-3 position-sweep lesson). Retries (re-sampling w entirely) if n <
    min_n or the half happens to be monochromatic."""
...
def audit_split(rows, split_name, marker_symbol):
    ...
    if hard_rows:
        ...
        def is_genuinely_mismatched(r):
            # COPY target (not reversal): after must differ from before itself
            m = r["s"].index(marker_symbol)
            before, after = r["s"][:m], r["s"][m + 1:]
        ...
        hard_checks.update({
            ...
            "all_genuinely_mismatched_copy": all(mismatched),
        ...
        hard_checks["swap_position_distribution_note"] = (
            ...
            "of the post-marker half -- roughly equal counts across bins would confirm "
            "positions are NOT concentrated at a single location."
        )
    ...
    if uniform_rows:
        ...
        uniform_checks.update({
            ...
            "note": (
                ...
                "one marker AND balanced halves AND correct copy content by chance)."
            ),
        })
...
def main():
    ...
    out = {
        "task": "marked-copy",
        "experiment": "corrected-dataset-fix (MCF1: dataset construction + audit)",
        "description": (
            "Corrected marked-copy dataset preserving Butoi et al.'s mixed negative-generation "
            "strategy (50% uniform-random / 50% adversarial) but replacing the K-edit-perturbation "
            "adversarial half with an explicit hard-negative construction: a genuine positive w#w "
            "with two post-marker positions swapped -- preserving marker count/position, balanced "
            "halves, and the post-marker half's content multiset, breaking only the literal "
            "copy-content match. Same construction template as marked-reversal's F1, adapted for "
            "COPY instead of REVERSAL. Swap position randomized per-example (uniform within the "
            "half) to avoid concentrating the content-mismatch signal at any single index."
        ),
        ...
        "known_simplifications_vs_original_pipeline": [
            ...
            "for the hard-negative half only.",
    ...
    out_path = RESULTS / "fix_dataset_marked_copy.json"
