"""Corrected-dataset experiment, MDF2: shortcut-only baseline ceiling on
the corrected test set.

Shortcut-only baseline: the FOUR features identified in phase3_missdup_
task_audit.py / reused for MDF1's construction (analysis.phase3_missdup_
task_audit.SHORTCUT_FEATURES) -- sequence_length_parity, blank_count_is_
one, aggregate_count_per_half, first_last_symbol_match -- each a trivial
counting/single-position check, no position-by-position content-carrier
structure. By MDF1's construction, every hard negative is independently
verified to pass all four (100% pass rate, verified in MDF1's audit after
fixing a j==0 boundary bug) -- so the entire hard-negative half is
structurally ambiguous by construction, matching the bucket-sort/marked-
copy ceiling formula exactly.

No pre-existing phase1 checkpoints exist for missing-duplicate-string, so
all three baselines (shortcut, ngram-logreg, bagcount-ffn) are TRAINED HERE
on missing-duplicate-string's STANDARD training data -- matching bucket-
sort's BSF2 / marked-copy's MCF2 convention.

PYTHONPATH=src:analysis python analysis/fix_baseline_ceiling_missing_duplicate_string.py
"""
...
from phase3_missdup_task_audit import SHORTCUT_FEATURES
...
FIXED_TEST_DIR = Path("languages/missing-duplicate-string-fixed/datasets/test")
LONG_SEQ_THRESH = 400
H_GRID = [8, 32, 128, 512]
...
def provable_incapacity_counterexample():
    """Same four shortcut-feature values, opposite validity -- the adjacent
    swap (indices 5,6, avoiding the j==0 boundary MDF1's construction
    excludes) breaks the true duplicate-of-halves check while preserving
    length, blank count, per-half aggregate counts, and first/last-symbol
    match."""
    clean = ["1", "1", "0", "1", "_", "1", "0", "1"]  # w+w, blank replaces a '1'; valid
    corrupted = ["1", "1", "0", "1", "_", "0", "1", "1"]  # positions 5,6 swapped; invalid
    ...
    return {
        ...
        "note": ("IDENTICAL four-feature vectors, opposite validity -- the information "
                "distinguishing them (whether the two halves genuinely match position-by-"
                "position, not just in aggregate bit counts) is structurally absent from any "
                "bag-of-counts-and-single-position representation, not merely hard to learn."),
    }
...
def train_shortcut_baseline():
    ...
    ckpt_path = CKPT_DIR / "phase1_missing_duplicate_string_shortcut_baseline.joblib"
...
def main():
    ...
    # ------------------------------------------------------------------
    # provable incapacity for content (position-by-position) verification
    # ------------------------------------------------------------------
    ...
    ceiling_report = {
        ...
        "formula": "(n_positive + n_structurally_obvious + 0.5 * n_structurally_ambiguous) / n_total "
                  ...
                  "four-feature bag-of-counts-and-single-position check) can catch; chance (0.5) "
                  "on every negative that passes all four listed shortcut features (by MDF1's "
                  "construction, this is the ENTIRE hard-negative half, plus any uniform-random "
                  "negatives that coincidentally also pass all four).",
    ...
    # ------------------------------------------------------------------
    print("\n=== training shortcut baseline on standard missing-duplicate-string training data ===", flush=True)
    ...
    sc_result["trained_on"] = "standard missing-duplicate-string training set"
    print(json.dumps(sc_result, indent=2, default=str))

    # ------------------------------------------------------------------
    # (2) ngram-logreg baseline -- TRAINED HERE
    # ------------------------------------------------------------------
    print("\n=== training ngram-logreg on standard missing-duplicate-string training data ===", flush=True)
    train_seqs, train_labels = load_split(TASK, "train")
    ...
    ng_ckpt_path = CKPT_DIR / "phase1_missing_duplicate_string_ngram_logreg.joblib"
    ...
    ng_result["trained_on"] = "standard missing-duplicate-string training set"
    ...
    bagcount_results = {}
    for h in H_GRID:
        print(f"\n=== training bagcount-ffn h={h} on standard missing-duplicate-string training data ===", flush=True)
        tB = train_candidate_B(TASK, h, vocab, train_seqs, train_labels, val_seqs, val_labels)
        ckpt_path = CKPT_DIR / f"phase1_missing_duplicate_string_bagcount_ffn_h{h}.pt"
        ...
        bc_result["trained_on"] = "standard missing-duplicate-string training set"
    ...
    out = {
        "task": TASK,
        "experiment": "corrected-dataset-fix (MDF2: shortcut-only baseline ceiling on corrected test data)",
        "description": (
            "Evaluates a NEWLY-TRAINED shortcut-only baseline (four-feature bag-of-counts-and-"
            "single-position, provably incapable of position-by-position content verification) "
            "plus ngram-logreg and bagcount-ffn baselines (also newly trained here, no pre-existing "
            "phase1 checkpoints for this task), all trained on STANDARD missing-duplicate-string "
            "training data, evaluated on the CORRECTED test set (MDF1), split into positive/"
            "uniform-random/hard-negative subsets."
        ),
    ...
    out_path = RESULTS / "fix_baseline_ceiling_missing_duplicate_string.json"
